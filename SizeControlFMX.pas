...
uses
  ...
  FMX.Menus,   //To hook the TSizeCtrl.PopupMenu
  FMX.TabControl, //To check the TTabSheet, TPageControl
  FMX.Objects,
  FMX.Types,
  System.Types,
  System.UITypes,
  ...
  (* [TSizeBtn reqs]
    ...
    Another way is to use TImageControl, but, it doesn't gives ability
    to use the Alpha-Blending :)
  *)

function getAbsoluteX(cntrl: TControl; LastControl: TControl): single;
function getAbsoluteY(cntrl: TControl; LastControl: TControl): single;
...
type
  ...
  TSizeBtn = class(TControl)
  private
    fTargetObj: TTargetObj;
    fPos: TBtnPos;
    fLeft, fTop: integer;
    fColor, fPen: TAlphaColor;
    fImage: TImage;
  protected
 ...
 {$IFNDEF VER3U} reintroduce; {$ENDIF}
  ...
  published
  function ClienToScreen(P: TPointF): TPointF;
  end;
  ...
  private
    ...
    procedure setBtnImage(Value: TImage);
    procedure setDisabledBtnImage(Value: TImage);
  ...
  function RegisteredCtrlFromPt(screenPt: TPoint;
    ...
    property BtnImage: TImage read fBtnImage write setBtnImage;
    //DisabledBtnImage - you will understand
    property DisabledBtnImage: TImage read fDisabledBtnImage write setDisabledBtnImage;
...
//------------------------------------------------------------------------------
function THackedControl.ClienToScreen(P: TPointF): TPointF;
begin
  Result := LocalToScreen(P);
end;
function getAbsoluteX(cntrl: TControl; LastControl: TControl): single;
begin
  ...
  if integer(cntrl.Parent) <> integer(LastControl) then
    Result := Result + getAbsoluteX(TControl(cntrl.Parent), LastControl);
end;
...
begin
  ...
  if integer(cntrl.Parent) <> integer(LastControl) then
    Result := Result + getAbsoluteY(TControl(cntrl.Parent), LastControl);
end;
...
begin
  ...
  while assigned(Control) do
    if TFMXObject(Control) is TCustomForm then
      exit
...
function GetBoundsAsScreenRect(Control: TFMXObject): TRect;
begin
  ...
  begin
    Result.TopLeft := THackedControl(parent).ClienToScreen(BoundsRect.TopLeft);
...
procedure TSizeBtn.MouseDown(Button: TMouseButton; Shift: TShiftState; X, Y: Single);
begin
  if Button = Left then
...
begin
  case fPos of
    bpLeft: Canvas.DrawPolygon(
      [Point(l, t+Floor(Height/2)),
      Point(l+Width-1, t),
      Point(l+Width-1, t+Height)
      ]);
    bpRight: Canvas.DrawPolygon(
...
begin
  if Assigned(fImage.Graphic) and (not fImage.Empty) then
...
begin
  Opacity := fTargetObj.fSizeCtrl.BtnAlphaBlend;
  Canvas.Fill.Color := fColor;
  if fPen = TAlphaColor.clNone then
...
begin
  ...
  begin
  if fBtns[i].Parent <> parentForm then
  begin
    fBtns[i].Parent := parentForm; //ie keep btns separate !!!
    st := true;
  end;
...
begin
  ...
  begin
    obj.Width := fFocusRect.Right - fFocusRect.Left;
    obj.Height := fFocusRect.Bottom - fFocusRect.Top;
...
begin
  if (not (aOwner is TControl)) and  (not (aOwner is TCustomForm)) then
    raise Exception.Create('TSizeCtrl.Create: Owner must be a Control');
  inherited Create(AOwner);
  fTargetList := TList.Create;
  fReIgnBtn := tszhNone;
  fRegList := TList.Create;
  fCanv := TCanvas.Create;
  fCanv.Fill.Color := clBtnFace;
  ...
  fBtnImage := TImage.Create;
  fDisabledBtnImage := TImage.Create;
  fMultiResize := True;
  ...
  begin
    ...
    for i := 0 to TargetCount - 1 do
      case TTargetObj(fTargetList[i]).fTarget.Align of
        TAlignLayout.None, TAlignLayout.Center: fValidBtns := fValidBtns - [bpLeft, bpTopLeft, bpTop,
            bpTopRight, bpRight, bpBottomRight, bpBottomLeft];
        TAlignLayout.Bottom: fValidBtns :=
            fValidBtns - [bpLeft, bpTopLeft, bpTopRight, bpRight,
            bpBottomRight, bpBottom, bpBottomLeft];
        TAlignLayout.Left: fValidBtns := fValidBtns - [bpLeft, bpTopLeft, bpTop,
            bpTopRight, bpBottomRight, bpBottom, bpBottomLeft];
        TAlignLayout.Right: fValidBtns :=
            fValidBtns - [bpTopLeft, bpTop, bpTopRight, bpRight,
            bpBottomRight, bpBottom, bpBottomLeft];
        TAlignLayout.Client: fValidBtns := [];
        TAlignLayout.Fit: fValidBtns := fValidBtns - [bpLeft, bpRight];
      end;
  end
  else
    fValidBtns := [];
...
procedure TSizeCtrl.SetBtnImage(Value: TImage);
begin
...
procedure TSizeCtrl.SetDisabledBtnImage(Value: TImage);
begin
